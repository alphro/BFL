...
#' and class \code{"BFL_score"} (returned by \code{\link{score_BFL}}), plus
#' \code{\link{site_similarity_BFL}} for inspecting pairwise site agreement.
#'
...
# ============================================================
# site_similarity_BFL — pairwise phi correlation across sites
# ============================================================

#' Pairwise site similarity from a BFL fit
#'
#' Computes pairwise Pearson correlations between local models using their
#' posterior-mean phi scores (per-record class probabilities) flattened
#' across all N observations and C causes.
#'
#' High correlation between two sites means their local models assign similar
#' scores to each target record — they are contributing redundant information
#' to the ensemble. Low correlation means the sites are complementary.
#' This is useful for diagnosing whether adding a site actually helps.
#'
#' @param fit Object of class \code{"BFL"} returned by \code{run_BFL()}.
#' @param plot Logical; if \code{TRUE}, prints a \code{ggplot2} heatmap of
#'   the correlation matrix. Default \code{FALSE}.
#'
#' @return An M \eqn{\times} M correlation matrix named by
#'   \code{fit$model_names}, returned invisibly. If \code{plot = TRUE},
#'   the heatmap is also printed.
#'
#' @seealso \code{\link{run_BFL}}, \code{\link{summary.BFL}}
#' @export
site_similarity_BFL <- function(fit, plot = FALSE) {

  stopifnot(inherits(fit, "BFL"))

  M   <- length(fit$model_names)
  NC  <- dim(fit$phi)[1] * dim(fit$phi)[2]   # N * C

  # Flatten phi (N x C x M) → (N*C x M) matrix, one column per site
  phi_flat <- vapply(seq_len(M),
                     function(m) as.vector(fit$phi[, , m]),
                     numeric(NC))
  colnames(phi_flat) <- fit$model_names

  cor_mat <- cor(phi_flat)

  if (isTRUE(plot)) {
    if (!requireNamespace("ggplot2", quietly = TRUE))
      stop("Package 'ggplot2' is required for plot = TRUE.")

    sites <- fit$model_names
    df <- data.frame(
      site_x = factor(rep(sites, times = M), levels = sites),
      site_y = factor(rep(sites, each  = M), levels = rev(sites)),
      r      = as.vector(cor_mat)
    )

    p <- ggplot2::ggplot(df, ggplot2::aes(x = site_x, y = site_y, fill = r)) +
      ...
      ggplot2::labs(
        title = "Site Similarity (phi correlation)",
        x     = NULL,
        y     = NULL
      ) +
      ggplot2::theme_minimal() +
