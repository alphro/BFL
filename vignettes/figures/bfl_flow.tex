...
\begin{tikzpicture}[font=\sffamily,
  ...
  \node[src] (ny) {\textbf{New York}\[1pt]\footnotesize local summary $\phi$};
  \node[src, below=0.8cm of ny] (se) {\textbf{Seattle}\[1pt]\footnotesize local summary $\phi$};
  \node[src, below=0.8cm of se] (au) {\textbf{Austin}\[1pt]\footnotesize local summary $\phi$};
  \node[agg, right=3.6cm of se] (bfl) {\textbf{BFL}\[1pt]aggregation\[1pt]\footnotesize learns weights $\lambda$};
  \node[tgt, right=3.2cm of bfl] (la) {\textbf{Los Angeles}\[1pt]\footnotesize target site\[1pt]\footnotesize estimated labels};
  \draw[arr] (ny.east) -- (bfl.west); \draw[arr] (se.east) -- (bfl.west); \draw[arr] (au.east) -- (bfl.west); \draw[arr] (bfl.east) -- (la.west);
  \node[font=\sffamily\bfseries] at ($(ny.north)+(0,0.5)$) {Source sites (fully labeled)};
  \node[lbl, align=center, above=0.2cm of bfl] {only summaries shared, no raw data};
\end{tikzpicture}
\end{document}
