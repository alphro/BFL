...
\begin{tikzpicture}[font=\sffamily,
  ...
  \node[chip, below=1.8cm of bfl] (lab) {\textbf{Los Angeles}};
  \draw[arr] (ny.east) -- (bfl.west); \draw[arr] (se.east) -- (bfl.west); \draw[arr] (au.east) -- (bfl.west); \draw[arr] (bfl.east) -- (la.west);
  \draw[arr] (lab.north) -- (bfl.south) node[midway, right=1pt, lbl]{into the model};
  \node[font=\sffamily\bfseries] at ($(ny.north)+(0,0.42)$) {Source sites (fully labeled)};
  \node[font=\sffamily\bfseries] at ($(lab.south)+(0,-0.35)$) {Target's partial labels};
\end{tikzpicture}
\end{document}
